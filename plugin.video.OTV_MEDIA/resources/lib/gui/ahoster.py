...
class HosterGui:
    ...
    def stream(self, playMode, siteName, function, url):
        ...
        self.dialog.create('OTV_MEDIA', 'get stream/hoster')
        ...
        if not siteResult:
            self.dialog.close()
            cGui().showInfo('OTV_MEDIA', 'stream/hoster not available')
            return
        ...
        if 'name' in siteResult[0]:
            ...
            if not siteResult:
                self.dialog.close()
                cGui().showInfo('OTV_MEDIA', 'no hoster available')
                return

            self.dialog.update(60, 'prepare hosterlist..')
            if (playMode != 'jd') and (playMode != 'jd2') and (playMode != 'pyload') and cConfig().getSetting('presortHoster') == 'true':
                # filter and sort hosters
                siteResult = self.__getPriorities(siteResult)
            if not siteResult:
                self.dialog.close()
                cGui().showInfo('OTV_MEDIA', 'no supported hoster available')
                return False
    ...
    def streamAuto(self, playMode, siteName, function):
        ...
        if 'name' in siteResult[0]:
            ...
            if len(siteResult) > self.maxHoster:
                siteResult = siteResult[:self.maxHoster - 1]
            check = False
            self.dialog.create('OTV_MEDIA', 'try hosters...')
            total = len(hosters)
