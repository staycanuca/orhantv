...
def main():
    ...
    try:
        ...
        if function == 'encode':
            ev = True if '--eval' in sys.argv  or  '-e' in sys.argv else False
            parent = True if '--parent' in sys.argv  or  '-p' in sys.argv else False

            print(jsfuck.encode(start_js, ev, parent))

        elif function == 'decode':
            print(jsfuck.decode(start_js))
