...
#vstream = xbmcaddon.Addon('plugin.video.OTV_MEDIA')
...
class cClear:
    ...
    def main(self, env):
        ...
        if (env == 'urlresolver'):
            ...
            try:
                sUrl = 'https://raw.githubusercontent.com/Kodi-vStream/venom-xbmc-addons/master/plugin.video.OTV_MEDIA/changelog.txt'
                oRequest =  urllib2.Request(sUrl)
                oResponse = urllib2.urlopen(oRequest)
                sContent = oResponse.read()
                self.TextBoxes('OTV_MEDIA Changelog', sContent)
            except:
            ...
            path = "special://home/addons/plugin.video.OTV_MEDIA"
            ...
            try:
                sUrl = 'https://raw.githubusercontent.com/Kodi-vStream/venom-xbmc-addons/master/plugin.video.OTV_MEDIA/soutient.txt'
                oRequest =  urllib2.Request(sUrl)
                oResponse = urllib2.urlopen(oRequest)
                sContent = oResponse.read()
                self.TextBoxes('OTV_MEDIA Soutient', sContent)
            except:
                self.DIALOG.VSerror("%s,%s" % (self.ADDON.VSlang(30205), sUrl))
            return
            ...
            if self.DIALOG.VSyesno("Êtes-vous sûr?"):
                ...
                cached_Cache = "special://home/userdata/addon_data/plugin.video.OTV_MEDIA/video_cache.db"
                #self.ClearDir2(cached_Cache, True)
                try:
                    xbmcvfs.delete(cached_Cache)
                    self.DIALOG.VSinfo('Clear Addon Cache, Successful[CR](Important relancer OTV_MEDIA)')
                except:
            ...
            cached_DB = "special://home/userdata/addon_data/plugin.video.OTV_MEDIA/OTV_MEDIA.db"
            ...
            if ret > -1:
                ...
                try:
                    ...
                    dbcur.close()
                    db.close()
                    self.DIALOG.VSok("Suppression BDD, Successful[CR](Important relancer OTV_MEDIA)")
                except:
            ...
            class XMLDialog(xbmcgui.WindowXMLDialog):
                ...
                def onInit(self):
                    ...
                    for aPlugin in aPlugins:
                        ...
                        icon = "special://home/addons/plugin.video.OTV_MEDIA/resources/art/sites/%s.png" % aPlugin[1]
            ...
            DB = "special://home/userdata/addon_data/plugin.video.OTV_MEDIA/OTV_MEDIA.db"
            if select >= 0:
                #new = self.DIALOG.browse(3, 'OTV_MEDIA', 'files')
                new = self.DIALOG.browse(3, 'OTV_MEDIA', "files")
                if new:
                    try:
                        if select == 0:
                            ...
                            xbmcvfs.copy(new + 'OTV_MEDIA.db', DB)
                        elif select == 1:
                            #copy(source, destination)--copy file to destination, returns true/false.
                            xbmcvfs.copy(DB, new + 'OTV_MEDIA.db')
