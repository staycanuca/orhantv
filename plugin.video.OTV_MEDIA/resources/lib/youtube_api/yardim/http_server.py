...
"""
    ...
    Copyright (C) 2018-2018 plugin.video.OTV_MEDIA
...
class YouTubeRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def __init__(self, request, client_address, server):
        self.addon_id = 'plugin.video.OTV_MEDIA'
        addon = xbmcaddon.Addon(self.addon_id)
        whitelist_ips = addon.getSetting('http.ip.whitelist')
    ...
    def do_GET(self):
        ...
        dash_proxy_enabled = addon.getSetting('mpd.videos') == 'true' and addon.getSetting('video.quality.mpd') == 'true'
    ...
    def do_POST(self):
        ...
        if not self.connection_allowed():
            ...
            license_url = xbmcgui.Window(10000).getProperty('plugin.video.OTV_MEDIA-license_url')
            license_token = xbmcgui.Window(10000).getProperty('plugin.video.OTV_MEDIA-license_token')
