...
def get_data_cache():
        ...
        if not _data_cache:
            ...
            _data_cache = max_file_size_mb=max_cache_size_mb
        return _data_cache
try:
...
from resources.lib.youtube_api.yardim.cipher import Cipher
...
class VideoInfo(object):
        ...
        for i in range(len(lines)):
            ...
            if re_match:
                ...
                if re_itag_match:
                    ...
                    if not yt_format:
                        log_debug('unknown yt_format for itag "%s"' % itag)
                        continue
    ...
    def _method_get_video_info(self, video_id):
        ...
        meta_info['subtitles'] = Subtitles(self._context, video_id, captions)
        ...
        for li_info in pa_li_info:
            if li_info.get('drmFamily') == 'WIDEVINE':
                ...
                if license_info['url']:
                    log_debug('Found widevine license url: |%s|' % license_info['url'])
        ...
        #if not is_live and  adaptive_fmts:
           # mpd_url, s_info = self.generate_mpd(video_id,adaptive_fmts,video_details.get('lengthSeconds', '0'),  cipher,license_info.get('url'))
    ...
    def generate_mpd(self, video_id, adaptive_fmts, duration, cipher,license_url):
        ...
        if not make_dirs(basepath):
            log_debug('Failed to create directories: %s' % basepath)
            return None

        has_video_stream = False
        ia_capabilities = inputstream_adaptive_capabilities()
        ...
        if not data.get('video/mp4') and not data.get('video/webm'):
            log_debug('Generate MPD: No video mime-types found')
            return None, None
        ...
        log_debug('Generating MPD: Apply filters |{apply_filters}| '
                ...
                for i in data[key]:
                    ...
                    if 'audio' in mime:
                        ...
                        if 'vp9.2' == video_codec.lower() and ('vp9.2' not in ia_capabilities 
                            ...
                            if not 'vp9.2' in ia_capabilities:
        ...
        log_debug('Generated MPD highest supported quality found: |%s|' % str(stream_info))
        if discarded_streams:
            ...
            log_debug('Generated MPD discarded streams: \n%s' % '\n'.join(str(stream) for stream in discarded_streams))

        if not has_video_stream:
            log_debug('Generated MPD no supported video streams found')
