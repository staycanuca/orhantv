...
the python script "\plugin.video.OTV_MEDIA\default.py" has left several classes in memory that we couldn't clean up. The classes include: class XBMCAddon::xbmcaddon::Addon

# https://stackoverflow.com/questions/26588266/xbmc-addon-memory-leak

"""
ADDONVS = xbmcaddon.Addon('plugin.video.sport365.live')    # singleton


class addon():


    def __init__(self, addonId = 'plugin.video.sport365.live'):
    ...
    def VSlang(self, lang):
        ...
        #Bug avec accent xbmc.translatePath(xbmcaddon.Addon('plugin.video.OTV_MEDIA').getLocalizedString(lang)).decode('utf-8')
...
# class dialog(xbmcgui.Dialog):
    ...
    def VSok(self, desc, title = 'OTV_MEDIA'):
        return DIALOG.ok(title, desc)

    def VSyesno(self, desc, title = 'OTV_MEDIA'):
        return DIALOG.yesno(title, desc)

    def VSselect(self, desc, title = 'OTV_MEDIA'):
    ...
    def VSinfo(self, desc, title = 'OTV_MEDIA', iseconds = 0, sound = False):
        if (iseconds == 0):
            iseconds = 1000
        else:
    ...
    def VSerror(self, e):
        return DIALOG.notification('OTV_MEDIA', 'Erreur: ' + str(e), xbmcgui.NOTIFICATION_ERROR, 2000), VSlog('Erreur: ' + str(e))

    def VStextView(self, desc, title = "OTV_MEDIA"):
...
class progress():
    ...
    def VScreate(self, title = 'OTV_MEDIA', desc = '', large=False):

        # Ne pas afficher le sablier si nous ne sommes pas dans un menu OTV_MEDIA
...
def VSlog(e, level = xbmc.LOGDEBUG):
    try:
        ...
        xbmc.log('\t[PLUGIN] OTV_MEDIA: ' + str(e), level)
