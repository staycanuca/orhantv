...
def showSearch():
    ...
    if (sSearchText != False):
        sUrl = 'https://www.hdfilmcehennemi2.pw/?s=' + sSearchText
        sUrl= sUrl.replace(' ','+')
        searchowMovies(sUrl)
...
def hdfilmcehennemi2(): #affiche les genres
    ...
    sUrl = 'https://www.hdfilmcehennemi2.pw/'

    sHtmlContent=getHtml(sUrl) 
    oParser = cParser()
    sPattern = '<a href="(https://www.hdfilmcehennemi2.pw/kategori/.*?)".*?>(.*?)</a>'
...
def showMovies(sSearch = ''):
    ...
    sHtmlContent=getHtml(sUrl) 
    sPattern = '<div class="moviefilm">.*?<a href="(.*?)".*?</i>(.*?)</span><img src="(.*?)" alt="(.*?)"'
    oParser = cParser()                  
    aResult = oParser.parse(sHtmlContent, sPattern)
