...
def showTV():
    ...
    sHtmlContent = cRequestHandler(MOV_TV).request()                                                     
    sPattern = '<div class="livetv_grid_item">.*?<a href=".*?" title="(.*?)".*?<div class="tv_logo" style="background-image:url\(\'(.*?)\'\);" alt="(.*?)">'
...
def showBox1():
    ...
    sTitle = alfabekodla(sTitle)
    urlk = "http://www.myvideo.az/ios/android.php?chan=aztv"
		                                                                                                                                                                                                  
    
    data=cRequestHandler(urlk).request() 
