# Embedded file name: C:\Users\ctubb\AppData\Roaming\Kodi\addons\script.module.swiftystreemz\resources\lib\peewee.py
...
__date_parts__ = set(('year',
 'month',
 'day',
 'hour',
 'minute',
 'second'))
__sqlite_datetime_formats__ = ('%Y-%m-%d %H:%M:%S',
 '%Y-%m-%d %H:%M:%S.%f',
 '%Y-%m-%d',
 '%H:%M:%S',
 '%H:%M:%S.%f',
 '%H:%M')
__sqlite_date_trunc__ = {'year': '%Y-01-01 00:00:00',
...
DJANGO_MAP = attrdict({'eq': operator.eq,
 'lt': operator.lt,
 'lte': operator.le,
 'gt': operator.gt,
 'gte': operator.ge,
...
def chunked(it, n):
    marker = object()
    for group in (list(g) for g in izip_longest(fillvalue=marker, *([iter(it)] * n))):
...
class Context(object):
    ...
    def value(self, value, converter = None, add_param = True):
        ...
        if add_param:
            return self.literal(self.state.param or '?')
        else:
            return self
    ...
    def __sql__(self, ctx):
        ...
        with ctx(parentheses=True):
            ...
            if self.start is not None and self.end is not None:
                ...
                parts.extend((SQL('%s BETWEEN' % frame),
                 self.start,
                 SQL('AND'),
                 self.end))
...
class BaseQuery(Node):
    ...
    def _get_cursor_wrapper(self, cursor):
        ...
        if row_type == ROW.TUPLE:
            return CursorWrapper(cursor)
        if row_type == ROW.NAMED_TUPLE:
            return NamedTupleCursorWrapper(cursor)
        if row_type == ROW.CONSTRUCTOR:
...
class CompoundSelectQuery(SelectBase):
    ...
    def _wrap_parens(self, ctx, subq):
        ...
        if not csq_setting or csq_setting == CSQ_PARENTHESES_NEVER:
            return False
        if csq_setting == CSQ_PARENTHESES_ALWAYS:
            return True
        if csq_setting == CSQ_PARENTHESES_UNNESTED:
...
IndexMetadata = collections.namedtuple('IndexMetadata', ('name',
 'sql',
 'columns',
 'unique',
 'table'))
ColumnMetadata = collections.namedtuple('ColumnMetadata', ('name',
 'data_type',
 'null',
 'primary_key',
 'table',
 'default'))
ForeignKeyMetadata = collections.namedtuple('ForeignKeyMetadata', ('column',
 'dest_table',
 'dest_column',
 'table'))
...
class SqliteDatabase(Database):
    field_types = {'BIGAUTO': FIELD.AUTO,
     'BIGINT': FIELD.INT,
     'BOOL': FIELD.INT,
     'DOUBLE': FIELD.FLOAT,
     'SMALLINT': FIELD.INT,
    ...
    def conflict_update(self, oc, query):
        if self.server_version < (3, 24, 0) and any((oc._preserve,
         oc._update,
         oc._where,
         oc._conflict_target,
...
class MySQLDatabase(Database):
    field_types = {'AUTO': 'INTEGER AUTO_INCREMENT',
    ...
    limit_max = 18446744073709551615L
    safe_create_index = False
...
class NamedTupleCursorWrapper(CursorWrapper):
    ...
    def initialize(self):
        description = self.cursor.description
        self.tuple_class = collections.namedtuple('Row', [ col[0][col[0].find('.') + 1:].strip('"') for col in description ])
...
class BigBitFieldData(object):
    ...
    def _ensure_length(self, idx):
        ...
        if cur_size <= byte_num:
            self._buffer.extend('\x00' * (byte_num + 1 - cur_size))
...
class ManyToManyField(MetaField):
    ...
    def _create_through_model(self):
        ...
        params = {'on_delete': self._on_delete,
         'on_update': self._on_update}
        attrs = {lhs._meta.name: ForeignKeyField(lhs, **params),
...
class Metadata(object):
    ...
    def __init__(self, model, database = None, table_name = None, indexes = None, primary_key = None, constraints = None, schema = None, only_save_dirty = False, depends_on = None, options = None, db_table = None, table_function = None, table_settings = None, without_rowid = False, temporary = False, legacy_table_names = True, **kwargs):
    ...
    def get_primary_keys(self):
        ...
        elif self.primary_key is not False:
...
class Model(with_metaclass(ModelBase, Node)):
    ...
    @classmethod
    def replace(cls, _Model__data = None, **insert):
...
class ManyToManyQuery(ModelSelect):
    ...
    def add(self, value, clear_existing = False):
        ...
        if isinstance(value, SelectQuery):
            ...
            if not value:
                return
            inserts = [ {accessor.src_fk.name: src_id,
             accessor.dest_fk.name: rel_id} for rel_id in self._id_list(value) ]
...
class PrefetchQuery(collections.namedtuple('_PrefetchQuery', ('query',
 'fields',
 'is_backref',
 'rel_models',
 'field_to_name',
